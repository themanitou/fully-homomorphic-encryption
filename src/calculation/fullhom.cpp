...
long BcastParams () {
    ...
    // workers compute R_POWERS and RATIO_POWERS
    if (FHE_MPI_RANK) {
        CompRPowers ();
        CompRatioPowers ();
    }
...
void FHE_Partial_Initialization () {
    ...
    // only the manager process compute the domain parameters
    if (FHE_MPI_RANK == 0) {
...
#ifdef FHE_LOG
        cout << "[Manager.Initialization] F=" << F << endl;
#endif
...
long WorkerPostProcess () {
    ...
    // receive the ciphertext from the manager
    MPI_ZZ_Recv (aCipher, 0, 0);

#ifdef FHE_MPI_LOG
    cout << "[worker-" << FHE_MPI_RANK << ".WorkerPostProcess] Received ciphertext from manager." << endl;
#endif
    ...
    do {
...
#ifdef FHE_MPI_LOG
        cout << "[worker-" << FHE_MPI_RANK << ".WorkerPostProcess] Received job k=" << k << " from manager." << endl;
#endif
        ...
        if (k) {
            ...
            // worker sends a command to the manager signaling that the result vectors will follow shortly
            FHE_MPI_Send_Command (k, 0);

#ifdef FHE_MPI_LOG
            cout << "[worker-" << FHE_MPI_RANK << ".WorkerPostProcess] k=" << k << " has been processed. Send signal to manager." << endl;
#endif

            // worker sends y_vector to manager
            MPI_ZZ_Send (y_vector, 0, k);

#ifdef FHE_MPI_LOG
            cout << "[worker-" << FHE_MPI_RANK << ".WorkerPostProcess] k=" << k << " has been processed. Send y_vector to manager." << endl;
#endif

            // worker sends z_vector to manager
            MPI_ZZ_Send (z_vector, 0, k);

#ifdef FHE_MPI_LOG
            cout << "[worker-" << FHE_MPI_RANK << ".WorkerPostProcess] k=" << k << " has been processed. Send z_vector to manager." << endl;
#endif
...
#ifdef FHE_MPI_LOG
    cout << "[worker-" << FHE_MPI_RANK << ".WorkerPostProcess] Ending post process." << endl;
#endif
...
long PostProcess (vec_ZZ& y_array, vec_ZZ& z_array, const ZZ& aCipher) {
    ...
    // manager sends command POST_PROCESS, along with the ciphertext
    k = 1;
    for (i = 1; i < FHE_MPI_SIZE; i++) {

        // command workers to enter PostProcess procedure
        FHE_MPI_Send_Command (FHE_MPI_COMMAND_POSTPROCESS, i);

        // send ciphertext to workers
        ...
        // send job k (possibly zero) to worker
    ...
    while (completed_count < SS_SIZE) {
        ...
        // manager receives signal from worker indicating that a row k has been done
...
#ifdef FHE_MPI_LOG
    cout << "[worker-" << FHE_MPI_RANK << ".WorkerRecrypt] Received ciphertext from manager." << endl;
#endif
...
#ifdef FHE_MPI_LOG
        cout << "[worker-" << FHE_MPI_RANK << ".WorkerRecrypt] Received job k=" << k << " from manager." << endl;
#endif
...
#ifdef FHE_MPI_LOG
            cout << "[worker-" << FHE_MPI_RANK << ".WorkerRecrypt] Received y_vector from manager." << endl;
#endif
...
#ifdef FHE_MPI_LOG
            cout << "[worker-" << FHE_MPI_RANK << ".WorkerRecrypt] Received z_vector from manager." << endl;
#endif
...
#ifdef FHE_MPI_LOG
            cout << "[worker-" << FHE_MPI_RANK << ".WorkerRecrypt] k=" << k << " has been processed. Send signal to manager." << endl;
#endif

            // worker sends qk_vector to manager
            MPI_VEC_ZZ_Send (qk_vector, 0, k);

#ifdef FHE_MPI_LOG
            cout << "[worker-" << FHE_MPI_RANK << ".WorkerRecrypt] k=" << k << " has been processed. Send qk_vector to manager." << endl;
#endif

            // worker sends linPart to manager
            MPI_ZZ_Send (linPart, 0, k);

#ifdef FHE_MPI_LOG
            cout << "[worker-" << FHE_MPI_RANK << ".WorkerRecrypt] k=" << k << " has been processed. Send linPart to manager." << endl;
#endif
...
long RecryptBit (ZZ& result,
    ...
    while (completed_count < SS_SIZE) {
        ...
        // manager receives the qk_vector
